...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    public string a_velocity, a_2DVelocity, a_Grounded, a_Idle, a_Jumped, a_Sliding, a_Sprinting, a_Crouching, a_AttackTrigger;
    ...
    void Update(){
        ...
        #region Input
            ...
            if(Mouse.current != null){
                attackInput_FrameOf = Mouse.current.leftButton.wasPressedThisFrame;
            }else{
                attackInput_FrameOf = false;
            }
            ...
            attackInput_FrameOf = Input.GetMouseButtonDown(0);
        ...
        switch (cameraPerspective){
            case PerspectiveModes._1stPerson:{
                if(_1stPersonCharacterAnimator){
                    ...
                    if(!zeroOut){
                        if(a_AttackTrigger != "" && attackInput_FrameOf){
                            _1stPersonCharacterAnimator.SetTrigger(a_AttackTrigger);
                        }
                    }else{
                        if(a_AttackTrigger != ""){
                            _1stPersonCharacterAnimator.ResetTrigger(a_AttackTrigger);
                        }
                    }
                }
            }break;
            ...
            case PerspectiveModes._3rdPerson:{
                if(_3rdPersonCharacterAnimator){
                    ...
                    if(!zeroOut){
                        ...
                        if(a_AttackTrigger != "" && attackInput_FrameOf){
                            _3rdPersonCharacterAnimator.SetTrigger(a_AttackTrigger);
                        }
                    }else{
                        if(a_velocity != ""){
                        ...
                        if(a_AttackTrigger != ""){
                            _3rdPersonCharacterAnimator.ResetTrigger(a_AttackTrigger);
                        }
                    }
...
public class SuperFPEditor : Editor{
    ...
    public override void OnInspectorGUI(){
        ...
        if(t._3rdPersonCharacterAnimator || t._1stPersonCharacterAnimator){
            ...
            t.a_AttackTrigger = EditorGUILayout.TextField(new GUIContent("Attack (Trigger)","(Trigger) The name of the Attack Parameter in the animator"),t.a_AttackTrigger);
            EditorGUILayout.EndVertical();
        }
        EditorGUILayout.HelpBox("Assign your animator parameters above. The controller automatically pushes movement booleans/floats every frame and fires the attack trigger when the left mouse button is pressed." ,MessageType.Info);
        EditorGUILayout.EndVertical();
